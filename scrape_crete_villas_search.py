...
def search_villas():
    # Gebruik Google Advanced Search parameters
    search_params = {
        'q': 'villa rental crete "contact us" OR "email" -booking.com -airbnb.com',
        'as_qdr': 'y',  # Laatste jaar
        'lr': 'lang_en',  # Engelse resultaten
        'cr': 'countryGR',  # Griekenland
        'num': 100  # Maximum resultaten per pagina
    }
    
    search_url = "https://www.google.com/search"
    
    try:
        print("Uitvoeren Google zoekopdracht...")
        response = requests.get(search_url, params=search_params, headers=get_headers())
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # Vind alle zoekresultaten
        ...
        for result in results:
            try:
                ...
                if link:
                    url = link['href']
                    if url.startswith('http') and not any(domain in url for domain in ['booking.com', 'airbnb.com', 'tripadvisor.com']):
                ...
                print(f"Fout bij resultaat: {str(e)}")
                continue
